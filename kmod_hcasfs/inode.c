...
struct inode* hcas_new_inode(struct super_block *sb, char hcas_object_name[HCASFS_OBJECT_NAME_LEN])
{
  ...
  result = hcas_lookup_object(sb->s_fs_info, hcas_object_name, &info->path);
  if (result) {
    kfree(info);
...
struct buffered_file *hcasfs_inode_buffered_file(struct inode *inode)
{
  ...
  if (!info->bf) {
    printk(KERN_INFO "Attempt open %p %p %p\n", info->path.dentry, info->path.mnt, info->path.dentry->d_inode);
...
static struct inode *_lookup_at_position(struct inode *dir, struct buffered_view *bv, u32 record_position, struct dentry *dentry) {
  char buf[96 + 256]; // entry size + NAME_MAX
  char *data;
  struct inode *inode;

  if (dentry->d_name.len > 255) {
    printk(KERN_WARNING "hcasfs: path name element longer than 255 bytes\n");
    return NULL;
  }

  loff_t pos = record_position;
  ...
  // TODO: May need to only read object name for nodes with objects in the future.
  ...
  if (S_ISDIR(inode->i_mode)) {
    ...
    if (S_ISREG(inode->i_mode)) {
      ...
      printk(KERN_INFO "hcasfs: lookup regular file\n");
    } else {
			     ...
			     unsigned int flags)
{
  ...
  while (lo < hi) {
    ...
    loff_t pos = 16 + 8 * ind;
    char *data = buffered_view_read_full(bv, dir_index_data, sizeof(dir_index_data), &pos);
...
static const char *hcasfs_get_link(struct dentry *dentry, struct inode *inode, struct delayed_call *done)
{
  struct buffered_view *bv;
  char *link_data;
  char *read_data;
  ...
  printk(KERN_INFO "hcasfs: Trying to read link %ld\n", inode->i_ino);

  if (unlikely(!S_ISLNK(inode->i_mode))) {
    return ERR_PTR(-EINVAL);
  }
  if (unlikely(inode->i_size > PATH_MAX)) {
    return ERR_PTR(-EIO);
  }
...
int hcasfs_inode_has_content(struct inode *inode) {
  return S_ISREG(inode->i_mode) || S_ISDIR(inode->i_mode) || S_ISLNK(inode->i_mode);
}