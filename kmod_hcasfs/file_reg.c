...
static int hcasfs_open(struct inode *inode, struct file *file)
{
  ...
  // TODO: Update this to use backing_file_open. Need to keep track of inode's user path separate from real_path of the upstream backing file.
  backing_file = backing_file_open(&file->f_path, O_RDONLY, &inode_info->path, current_cred());
  if (IS_ERR(file)) {
...
static ssize_t hcasfs_read_iter(struct kiocb *iocb, struct iov_iter *iter)
{
  ...
  struct backing_file_ctx ctx = {
    ...
    // TODO: Do I need to ensure atime updates?
    //.accessed = hcasfs_file_accessed,
  };
  printk(KERN_INFO "hcasfs: calling read_iter\n");
  if (!iov_iter_count(iter))
    return 0;

  return backing_file_read_iter(file_data->backing_file, iter, iocb, iocb->ki_flags, &ctx);
}
...
static int hcasfs_fadvise(struct file *file, loff_t offset, loff_t len, int advice)
{
             ...
             unsigned int flags)
{
  ...
  iocb.ki_pos = *ppos;
  ret = backing_file_splice_read(file_data->backing_file, &iocb, pipe, len, flags, &ctx);
  *ppos = iocb.ki_pos;
