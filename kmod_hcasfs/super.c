...
static int hcas_parse_hex_name(char *hex_name,
...
int hcasfs_parse_options(char *options, struct hcasfs_sb_info *sbi)
{
	...
	if (!options) {
		printk(KERN_ERR
		       "hcasfs: Missing required option: root_object\n");
		return -EINVAL;
	}
	...
	while ((p = strsep(&options, ",")) != NULL) {
		...
		case Opt_root_object:
			...
			if (!obj_name)
				return -ENOMEM;
			if (hcas_parse_hex_name(obj_name,
						sbi->root_object_name)) {
				printk(KERN_ERR
				       "hcasfs: Failed to parse root_object name\n");
				return -EINVAL;
			}
			break;
	...
	if (root_object_found > 1) {
		printk(KERN_ERR
		       "hcasfs: root_object can only be provided once\n");
		return -EINVAL;
	}
...
int hcasfs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	memcpy(&sbi->hcas_data_dir, &mount_data->hcas_data_dir,
	       sizeof(struct path));
...
static void hcas_build_object_path(char *path_buf,
				   char obj_name[HCASFS_OBJECT_NAME_LEN])
{
	path_buf[2] = '/';
	for (int i = 0; i < HCASFS_OBJECT_NAME_LEN; i++) {
		int offset = 2 * i;
		...
		path_buf[offset + 1] =
			hcas_nibble_to_hex_digit(obj_name[i] & 0xf);
	}
